...
void MainWindow::on_mostrarPuntos_clicked()
{
    list<Neurona>& lista = listaNeuronas.listaNeurona();
    QGraphicsScene* g = new QGraphicsScene;
    ui->grafica->setScene(g);
    ...
    for (Neurona &neurona : lista) {
        ...
        QPen pen(color);
        QBrush brush(color);
        g->addEllipse(neurona.getPosX(), origen, 5, 5, pen, brush);
    }
}

void FuerzaBrutaBuscarCercania(int &x, int &y, list<Neurona>& lista, Neurona &n){
    double distancia, distanciaMin = 0;
    for(Neurona &neurona : lista ){
        distancia = sqrt(pow(n.getPosX() - neurona.getPosX(), 2) + pow(n.getPosY() - neurona.getPosY(), 2));
        if(distancia < distanciaMin){
            distanciaMin = distancia;
            x = neurona.getPosX();
            y = neurona.getPosY();
        }else if(distanciaMin == 0 && n.getId() != neurona.getId()){
            distanciaMin = distancia;
            x = neurona.getPosX();
            y = neurona.getPosY();
        }
...
void MainWindow::on_enlazar_clicked()
{
    int x = 0, y = 0;
    list<Neurona>& lista = listaNeuronas.listaNeurona();
    QGraphicsScene* g = new QGraphicsScene;
    ...
    for (Neurona &neurona : lista) {
        ...
        QBrush brush(color);
        g->addEllipse(neurona.getPosX(), origen, 5, 5, pen, brush);
        FuerzaBrutaBuscarCercania(x, y, lista, neurona);
        qreal origen2 = alto - y;
        g->addLine(neurona.getPosX(), origen, x, origen2, pen);
    }
}
